...
1. `buildOrderFromLines` re-prices every line from the current `products` table (never trusts a client-sent subtotal/total) and pre-checks `variant_stock` for each `{color,size}` — optimistic, re-checked again in step 3.
...
3. `paymentService.chargeCard(...)` charges Wompi **before any DB write** — a decline or Wompi-side failure means no `orders` row is ever created. The Wompi integrity `signature` is computed server-side (`SHA-256(reference+amountInCents+currency+WOMPI_INTEGRITY_KEY)`), and the Wompi request carries an `Idempotency-Key` equal to the server-generated `reference`.
4. On approval, one `pool.getConnection()` transaction does: re-check stock with `SELECT ... FOR UPDATE`, insert `orders`/`order_items`, decrement `products.variant_stock`, insert one `shipments` row (`status='pending'`), insert `payment_transactions`. If this transaction fails **after** Wompi already charged (e.g. a lost stock race), there is no automatic Wompi reversal — it's logged at `fatal-payment-orphan` with the Wompi transaction id and returned to the client as a 500 telling them to contact support with that id for a manual refund. This mirrors a known gap in the reference implementation (`etniapp-core`) rather than inventing an untested auto-reversal call.
...
6. After commit, `emailService.sendOrderConfirmation(...)` (Resend) runs best-effort in its own `try/catch` — a failure here is logged and never changes the 201 response or rolls back the already-paid order.
